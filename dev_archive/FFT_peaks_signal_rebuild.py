...
idx_start = idx_end - 2
plt.scatter(ltime[idx_start:idx_end+1],
            lenv[idx_start:idx_end+1], 20, marker='x', color='red')
...
(a, b), _ = curve_fit(onset_exp, ltime[idx_start:idx_end+1],
                      lenv[idx_start:idx_end+1])
...
plt.scatter(ltime[idx_start:idx_end+1], lenv[idx_start:idx_end+1],
            20, marker='x', color='blue')
...
coefs, _ = curve_fit(decay_poly, ltime[idx_start:idx_end+1],
                     lenv[idx_start:idx_end+1], sigma=sigma)
...
plt.annotate(r'$env = ax^7 + bx^6 + cx^5 + dx^4 +ex^3 + fx^2 + gx + h$',
             (14e-2, 0.8), fontsize=13)

#########################################################
# Signal Reconstruction from Fast Fourier Transform Peaks
#########################################################
...
t = t[t < 3]
new_s = 0
for f, amp in zip(peak_freqs, amps):
    new_s += amp*np.sin(f*t*2*np.pi)
    
idx = np.where(t > 1e-1)[0][0]
onset_env = onset_exp(t[:idx], a, b)
new_s[:idx] = onset_env*new_s[:idx]
decay_env = decay_poly(t[idx:], *coefs)
new_s[idx:] = decay_env*new_s[idx:]
new_s *= 1/np.max(new_s)

plt.figure()
plt.plot(t, new_s, label='reconstructed')
s.signal.normalize().trim_time(3).plot.signal(label='original')
plt.legend()

print('Reconstructed signal')
sig = guitarsounds.Signal(new_s, s.signal.sr,
                          guitarsounds.parameters.sound_parameters())
sig.listen()
