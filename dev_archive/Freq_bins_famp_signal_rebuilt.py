...
pfreqs = s.signal.fft_frequencies()[peaks]
pamps = s.signal.fft()[peaks]
a2 = scipy.interpolate.InterpolatedUnivariateSpline(pfreqs, pamps, k=2)
plt.plot(freqs, a2(freqs), label='spline', color='orange')

a3 = scipy.interpolate.interp1d(pfreqs, pamps, bounds_error=False,
                                fill_value=(pamps[0], pamps[-1]))
...
a0 = scipy.interpolate.interp1d(pfreqs, pamps, bounds_error=False,
                                fill_value=(pamps[0], pamps[-1]))
freq = s.fundamental
time = s.signal.time()
time = time[time < 1]
...
for b in s.bins:
    ...
    bin_env = scipy.interpolate.interp1d(tim, env)
    sig_e = 0
    while freqs[i] < mf:
...
for i, b in enumerate(s.bins):
    s.bins[b].trim_time(1).normalize().plot.log_envelop(linestyle='--',
                                                        color=gen_cm(colorange[i]), alpha=0.6)
